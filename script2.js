    ...
    event.target.innerText = event.target.innerText
      ...
      .map((letter, index) => {
        ...
        if(index < iteration) {
      	...
      	// Otherwise, return a random letter from the letters string
        return letters[Math.floor(Math.random() * 26)]
      })
    
    	// Join the array of characters back into a string and set it as the new text of the target element
      .join("");
